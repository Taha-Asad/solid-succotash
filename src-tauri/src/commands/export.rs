...
mod tests {
    ...
    use crate::commands::invoices::{add_invoice_item, create_customer, create_invoice, finalize_invoice};
    ...
    async fn stock_csv_writes_header_and_rows() {
        ...
        let returned = export_stock_csv(app.state(), app.state(), path.to_string_lossy().to_string())
            .await
            .expect("export");
    ...
    async fn sales_csv_writes_rows() {
        ...
        assert!(
            content.contains(&format!("{},2026-01-15,Walk-in,2000,0,2000,finalized", invoice.invoice_number)),
            "got:\n{content}"
        );
        std::fs::remove_file(&path).ok();
    }
    ...
    async fn sales_csv_denied_for_employee() {
        ...
        .await
        .expect("company id");
        let employee = insert_user(&pool, &company_id, "e2@test.com", "Emp", "employee", true).await;
        set_session_user(&app, employee).await;
